...
- **Markdown files** (`.md`, `.markdown`, `.mdown`, `.mkd`) open in the rendered view; the two-sided **Preview | Diff** pill in their panel header switches to the diff and back.
- **The Description panel** (when the range carries a notes commit — see the notes-commit convention in [CLI.md](CLI.md)) sits above Commits and renders `PR-DESCRIPTION.md` as the page's PR description. It is a "fake file": not part of the reviewed diff, but fully commentable — click a rendered block and the comment anchors to `PR-DESCRIPTION.md`, `New` side, the block's 1-based source line, flowing through storage and exports like any other comment. The rendered view is diff-aware: added runs are tinted green, removed runs red, unchanged context is plain. For modified files it covers the diff hunks only, with `⋯` markers where unchanged text is elided; added files render in full. Deleted and binary files have no rendered view. A markdown construct straddling a change boundary renders as separate blocks — inherent to rendering a diff.
